...
namespace Rubickanov.EQS
{
    ...
    public class EQSQuery
    {
        ...
        private EQSQueryResult _result = EmptyResult;
        ...
        /// <summary>
        /// The items produced by the generator. Read-only snapshot, valid after <see cref="Start"/>;
        /// do not cast back to a mutable collection and do not hold it across query runs.
        /// </summary>
        ...
        {
            if (_status == EQSQueryStatus.NotStarted)
                throw new InvalidOperationException(
                    "EQSQuery.Tick() called before Start(). Call Start(context) first.");
            ...
            while (_currentTestIndex < tests.Count)
            {
                ...
                if (!filterOnly)
                {
                    ...
                    for (int i = 0; i < itemCount; i++)
                    {
                        ...
                        if (score > 1f) test.WarnOutOfRangeOnce();
#endif
                ...
                // Early exit by domination: prune items that can't possibly out-score the
                // current leader. Only sound when no remaining test can filter — otherwise a
                // later test could discard the leader, and a pruned item would have been the
                // real winner. Skip pruning entirely if any remaining test may filter.
                float remainingWeight = 0f;
                bool anyRemainingCanFilter = false;
                for (int t = _currentTestIndex + 1; t < tests.Count; t++)
                {
                    var ft = tests[t];
                    if (ft == null) continue;
                    if (!ft.NeverFilters) anyRemainingCanFilter = true;
                    if (ft.ScoreMode == EQSTestScoreMode.FilterOnly) continue;
                    remainingWeight += ft.Weight;
                }

                if (!anyRemainingCanFilter && remainingWeight > 0f)
                {
        ...
        private void BuildResult()
        {
            ...
            // Hand out the reused buffer directly — no per-query allocation. The result is
            // only valid until the next Start()/Reset()/RunSync() on this query (see EQSQueryResult).
            _result = new EQSQueryResult(_resultItems.Count > 0, _resultItems);
        }

        private static readonly EQSQueryResult EmptyResult =
            new(false, Array.Empty<EQSScoredItem>());
    }
}
