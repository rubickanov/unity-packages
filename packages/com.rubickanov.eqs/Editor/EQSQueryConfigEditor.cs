...
namespace Rubickanov.EQS.Editor
{
    ...
    public class EQSQueryConfigEditor : UnityEditor.Editor
    {
        ...
        // Keyed by managedReferenceId, not list index — index-keyed state misaligns the moment
        // a test is moved or removed.
        private readonly System.Collections.Generic.HashSet<long> _testFoldouts = new();
        ...
        private static void CacheTypes()
        {
            _generatorTypes = AppDomain.CurrentDomain.GetAssemblies()
                ...
                .OrderBy(t => t.Name)
                .ToArray();
            _generatorTypeNames = _generatorTypes.Select(MenuName).ToArray();
            ...
            _testTypes = AppDomain.CurrentDomain.GetAssemblies()
                ...
                .OrderBy(t => t.Name)
                .ToArray();
            _testTypeNames = _testTypes.Select(MenuName).ToArray();
        }

        // Namespace as a submenu path so types with the same simple name in different namespaces
        // stay distinguishable in the Add menu (GenericMenu treats '/' as a separator).
        private static string MenuName(Type t) =>
            string.IsNullOrEmpty(t.Namespace) ? t.Name : t.Namespace.Replace('.', '/') + "/" + t.Name;
        ...
        private void DrawTest(int index)
        {
            ...
            long key = element.managedReferenceId;
            bool expanded = _testFoldouts.Contains(key);

            EditorGUILayout.BeginHorizontal();

            expanded = EditorGUILayout.Foldout(expanded, typeName, true);
            if (expanded) _testFoldouts.Add(key); else _testFoldouts.Remove(key);
