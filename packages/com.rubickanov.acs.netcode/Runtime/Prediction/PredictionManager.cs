...
{
    /// <summary>
    /// Non-generic reset dispatcher for <see cref="PredictionManager{TInput}"/>. Unity does not
    ...
    /// </remarks>
    [Preserve]
    internal sealed class PredictionManager<TInput> : IAspectPredictionHook, IDisposable
        where TInput : unmanaged, IInputCommand
    {
        ...
        // (which Unity does not fire on generic classes) can still reach this closed generic's
        // static state. Fires once per closed TInput per domain lifetime.
        static PredictionManager()
        {
            PredictionManagerReset.RegisterClearer(() =>
            {
                s_Systems.Clear();
                s_warnedZeroTickRate = false;
            });
        }
        ...
        private PredictionManager(NetworkManager networkManager)
        {
            ...
            // TickRate == 0 is rejected in GetOrCreate before we reach the ctor —
            // so here we can assume a positive tick rate and divide directly.
        ...
        internal static PredictionManager<TInput>? GetOrCreate(NetworkManager networkManager)
        {
            ...
            if (tickRate == 0)
            {
                if (!s_warnedZeroTickRate)
                {
                    s_warnedZeroTickRate = true;
                    Debug.LogError("[PredictionManager] NetworkTickSystem.TickRate is 0 — refusing to build prediction manager. Set a non-zero tick rate in NetworkConfig before spawning predicted entities.");
                }
                return null;
            }

            var system = new PredictionManager<TInput>(networkManager);
