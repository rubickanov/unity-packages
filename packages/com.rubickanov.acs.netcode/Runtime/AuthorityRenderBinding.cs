...
namespace Rubickanov.ACS.Runtime.Netcode
{
        ...
        where T : unmanaged
    {
        ...
        // (visible jitter). 10ms is well below any realistic tick interval (≥30Hz = 33ms)
        // but orders of magnitude above intra-frame write spread (<1ms).
        ...
        // Sized at ~2 ticks @ 30 Hz (66ms) — any realistic motion streak produces writes
        // much faster than this, so continuous motion never falls into the bootstrap branch.
        ...
        public AuthorityRenderBinding(ReactiveProperty<T> reactive, Lerp<T> lerp)
            : base(reactive)
        {
            _lerp = lerp;
        ...
        private void RecordSample(T value)
        {
            ...
            if (gap > StaleSampleThresholdSeconds)
            {
                _curr = value;
                _currTime = now;
                _interpolatedValue = value;
