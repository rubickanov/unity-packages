...
namespace Rubickanov.BehaviorTree.Tests
{
    ...
    public class BehaviorTreeAssetTests
    {
        ...
        [Test]
        public void CreateInstance_PreservesGuidsForEveryNode()
        {
            // The runtime tree mirrors the asset GUIDs so the editor can highlight live
            // node states in play mode by matching runtime nodes to their views.
            ...
            Assert.AreEqual("seq-src", clone.Guid);
            Assert.AreEqual("inv-src", cloneInverter.Guid);
            Assert.AreEqual("leaf-src", cloneLeaf.Guid);
        }
