...
public static class BehaviorTreeAutoLayout
{
    ...
    public static void Layout(BehaviorTreeSerializer serializer)
    {
        ...
        if (rootGuid != null)
            LayoutRecursive(rootGuid, 0, childMap, positions, visited, ref nextX);
        ...
        if (orphanRoots.Count > 0)
        {
            ...
            foreach (var orphanGuid in orphanRoots)
            {
                ...
                {
                    LayoutRecursive(orphanGuid, 0, childMap, positions, visited, ref orphanX);
                    OffsetSubtree(orphanGuid, childMap, positions, new Vector2(0, orphanY), new HashSet<string>());
                }
                else
                {
        ...
        HashSet<string> visited)
    {
        ...
        if (childMap.TryGetValue(guid, out var children))
        {
            foreach (var childGuid in children)
                OffsetSubtree(childGuid, childMap, positions, baseOffset, visited);
        }
    }
    ...
    {
        if (!visited.Add(guid))
            return positions.TryGetValue(guid, out var existing) ? existing.x : 0f;
        ...
        foreach (var childGuid in children)
        {
            float childX = LayoutRecursive(childGuid, depth + 1, childMap, positions, visited, ref nextX);
