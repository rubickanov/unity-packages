...
public class BehaviorTreeGraphView : GraphView
{
    ...
    public void PopulateView(BehaviorTreeSerializer serializer)
    {
        ...
        var savedMatrix = viewTransform.matrix;
        var savedPosition = (Vector3)savedMatrix.GetColumn(3);
        var savedScale = savedMatrix.lossyScale;
    ...
    {
        if (string.IsNullOrEmpty(node.Guid) || map.ContainsKey(node.Guid)) return;
